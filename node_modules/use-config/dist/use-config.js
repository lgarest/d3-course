...
class UseConfig {
      ...
      loaderContext = this.getLoaderContext({ sync: false });
      {
        ...
        function $Loop_2() {
          if (!($iterator_index__filename_1[1] = $iterator_index__filename_1[0].next()).done && (([index, _filename] = $iterator_index__filename_1[1].value) || true)) {
            ...
            return pathExists(filepath).then(function ($await_4) {
              try {
                ...
                return loader(filepath).then(function ($await_5) {
                  try {
                    config = $await_5;
  ...
  loadSync() {
    ...
    for (var i = 0, list = this.options.files.entries(); i < list.length; i += 1) {
      const ref = list[i];
      var index = ref[0];
      var _filename = ref[1];
